...
const KEY_LEGEND: { keys: string; label: string }[] = [
  ...
  { keys: "Q / W", label: "Seleccionar SÍ / NO" },
...
export function ControllerInspector({
  ...
  const questionCount =
    component.groups[component.groupIndex]?.questions.length ?? 0;
      ...
      {component.fileName ? (
        <div className="flex flex-col gap-1 rounded-md bg-elev p-2 text-2xs text-dim">
          ...
          <span className="mt-1 text-ink">
            ...
            Pregunta {questionCount ? component.questionIndex + 1 : 0}/
            {questionCount} · Selección:{" "}
            {component.selected >= 0 ? OPTION_LABELS[component.selected] : "—"}
          </span>
        </div>
      ) : (
